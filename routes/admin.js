...
const fs = require('fs');
const htmlTitle = "ERB Hobby Workshop";
...
router.post('/deleteClassUser/:uID/:classID', async function (req, res, next) {
    ...
    try {
        ...
        // add credit
        let classCredit = await db.collection("class").find({classID: Number(req.params.classID)}).toArray();
        classCredit = classCredit[0].classCredit;
        const user = await db.collection("user").find({ uID: Number(req.params.uID) }).toArray();
        user[0].Credit = user[0].Credit + classCredit;
        const updateCredit = await db.collection("user").replaceOne({ uID: Number(req.params.uID) },user[0])
...
router.get('/rpt_userRegMM/', async function (req, res) {
    if (req.isAuthenticated()) {
        if (req.session.passport.user.uLevel == 1) {
            res.render("rpt_userRegMM", {
                title: htmlTitle,
                user: req.session.passport.user,
            });
        } else {
